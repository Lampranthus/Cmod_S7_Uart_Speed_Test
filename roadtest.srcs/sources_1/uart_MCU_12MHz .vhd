...
ENTITY uart_MCU_12MHz IS
    GENERIC(
        clk_freq        :    INTEGER        := 360_000_000;    --frequency of system clock in Hertz
        baud_rate    :    INTEGER        := 12_000_000;        --data link baud rate in bits/second
        os_rate        :    INTEGER        := 10;            --oversampling rate to find center of receive bits (in samples per baud period)
...
ARCHITECTURE logic OF uart_MCU_12MHz IS
                ...
                WHEN idle =>                                                                        --idle state
                    ...
                    IF(rx = '0') THEN                                                                 --start bit might be present
                        ...
                        ELSE                                                                                    --oversampling pulse counter is at bit center
                            os_count := 0;                                                                        --clear oversampling pulse counter
                            rx_count := 0;  
                            rx_buffer <= (OTHERS => '0');                                                                      --clear the bits received counter
